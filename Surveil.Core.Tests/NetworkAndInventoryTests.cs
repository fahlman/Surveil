...
public sealed class NetworkAndInventoryTests
{
    [Fact]
    ...
    public void DiffsNewPresentAndMissingWithinScope()
    {
        ...
        Assert.Equal(CameraPresenceStatus.Present, statuses.Single(item => item.Ip == "192.168.50.5").Presence);
        Assert.Equal(CameraPresenceStatus.Absent, statuses.Single(item => item.Ip == "192.168.50.6").Presence);
        Assert.Equal(CameraPresenceStatus.New, statuses.Single(item => item.Ip == "192.168.50.7").Presence);
