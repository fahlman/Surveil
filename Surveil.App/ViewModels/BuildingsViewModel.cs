...
public sealed partial class BuildingsViewModel : ObservableObject
{
    ...
    {
        Buildings.Clear();
        foreach (var building in session.Config.Buildings) Buildings.Add(building);
        SelectedBuilding = Buildings.FirstOrDefault();
    }

    partial void OnSelectedBuildingChanged(Building? oldValue, Building? newValue)
    {
    ...
    {
        var building = new Building { Name = $"Building {Buildings.Count + 1}" };
    ...
    private async Task SaveAsync()
    {
        ...
        {
            CommitCurrent();
            var config = new SurveilConfig { Buildings = Buildings.ToList() };
    ...
    public async Task ExportAsync(string path)
    {
        ...
        {
            CommitCurrent();
            session.Config = new SurveilConfig { Buildings = Buildings.ToList() };
    ...
    private void CommitRanges(Building building) => building.Ranges = Ranges.ToList();
}
